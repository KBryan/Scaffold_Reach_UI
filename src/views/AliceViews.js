...
export class Deploy extends React.Component {
  render() {
    ...
    return (
      <div>
        Deploy the contract and pay transaction fee.
        <br />
...
export class EnterInfo extends React.Component {
  render() {
    ...
    return (
      <div>
          <h3 id="addr">0x34535jkl534535jkl345jkl</h3>
          Upload Image:<input/>
          <button id="upload_image">Upload Image</button><br/><br/>
        <textarea
          onChange={(e) => this.setState({info: e.currentTarget.value})}
          placeholder={defaultInfo}
        />
        <br />
        <button id="button_style" onClick={() => parent.enterInfo(info || defaultInfo)}
